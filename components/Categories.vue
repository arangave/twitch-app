...
.category-card {
  display: flex; flex-direction: column; gap: 0.5rem; background: $background-dark; border-radius: 0.5rem; overflow: hidden;
  transition: box-shadow 0.3s, transform 0.3s;
  &:hover { box-shadow: 0 0 0.625rem 0.125rem $primary-color; transform: translateY(-0.125rem); }
  &__image { width: 100%; height: auto; object-fit: cover; border-bottom: 0.0625rem solid $background-dark; }
  &__info { padding: 0.5rem; }
  &__name { font-weight: 600; font-size: 0.9rem; color: $text-light; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 0.4rem; }
  &__viewers { color: $text-muted; font-size: 0.8rem; margin-top: 0; }
...
.featured-category {
  width: 100%; max-width: 16rem; height: 3.3125rem; background: $primary-color; border-radius: 0.375rem; color: $text-light; font-weight: 500;
  display: flex; align-items: center; justify-content: space-between; padding: 0 0.8rem; font-size: 1.2rem;
  transition: background-color 0.3s ease, transform 0.2s ease;
  span { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  img { width: 4rem; height: 4rem; object-fit: contain; }
  &:hover { background-color: $hover-color; transform: translateY(-0.125rem); }
  @include respond('mobile') {
    font-size: 1rem; max-width: 14rem; height: 2.4rem; padding: 0 0.6rem; align-items: center;
